...
import com.kahavanu.data.goals.toGoalLogEntity
...
@Singleton
class GoalsSyncManager @Inject constructor(
    ...
    private suspend fun pushPendingGoals(uid: String) {
        val dao = database.goalLogDao()
        dao.getUnsynced(uid).forEach { entity ->
            val collection = firestore.collection("users").document(uid).collection("goalLogs")
    ...
    private suspend fun pullRemoteGoals(uid: String) {
        val dao = database.goalLogDao()
        val snapshot = firestore
            .collection("users").document(uid).collection("goalLogs")
            .get()
            .awaitResultQuery()
            .getOrNull() ?: return
